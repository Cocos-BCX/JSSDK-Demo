...
参数：  
[TAB]file：宿主环境若在web => file input 的 change事件触发返回的事件对象event.target，files[0]  
[TAB]callback：回调函数  
...
参数：  
[TAB]privateKey：明文私钥  
[TAB]password：如果是已经创建钱包或恢复钱包，此时的密码为原来钱包的密码，否则是可以随意填写的临时密码  
[TAB]callback：回调函数  
...
[TAB]直接返回数据，格式示例：  
```js  
[TAB][TAB]"accounts": ["tom0002"],  
[TAB][TAB]"currentAccount": {  
[TAB][TAB][TAB]"userId": "1.2.20",  
[TAB][TAB][TAB]"isLocked": true,  
[TAB][TAB][TAB]"name": "tom0002"  
[TAB][TAB]}  
```  
...
[TAB]account：用户名  
[TAB]password：密码  
[TAB]autoLogin：boolean类型，指定是否自动登录，默认值为false  
[TAB]callback：回调函数  
...
参数：  
[TAB]callback：设置获取私钥成功后的回调函数，回调参数result为Object对象，  
[TAB]对象结构为：  
```js  
[TAB][TAB]{  
[TAB][TAB][TAB]status:1,data:{  
[TAB][TAB][TAB]activePrivateKey:”xxxxxxxxxxx”  
[TAB][TAB][TAB]}  
[TAB][TAB]}  
```  
...
参数：  
[TAB]account：账户名  
[TAB]callback：只要用户操作记录有变化，就调用此callback：回调函数  
...
功能：向目标对象发送代币  
参数：  
[TAB]fromAccount：发款方账户名，不是发起提议，  
[TAB]toAccount：收款方账号名  
[TAB]amount：发送的代币数量  
[TAB]assetId：资产ID （如：X.X.X）或 代币符号（如：BTC）  
[TAB]memo：转账备注  
[TAB]feeAssetId：支付手续费的代币资产符号  
[TAB]isPropose：是否发起提议  
[TAB]onlyGetFee（boolean）：是否只获取本次操作所需手续费  
[TAB]callback：设置转账后的回调函数  
  
### 创建资产  
方法：createAsset  
功能：创建token  
参数：  
[TAB]assetId：资产符号，正则^\[\.A-Z\]+$  
[TAB]precision：精度(小数位数)  
[TAB]maxSupply：最大资产总量  
[TAB]description: 资产描述，可不填  
[TAB]onlyGetFee：设置只返回本次调用所需手续费  
[TAB]callback：见统一API说明  
[TAB]coreExchangeRate(Object)：  
```js  
[TAB][TAB]{  
[TAB][TAB][TAB]quoteAmount:标价资产(即创建的代币，默认1),  
[TAB][TAB][TAB]baseAmount: 基准资产(即核心资产，默认1)  
[TAB][TAB]}  
```  
...
[TAB]onlyGetFee：设置只返回本次调用所需手续费  
[TAB]callback：见统一API说明  
[TAB]coreExchangeRate(Object)：手续费汇率  
```js  
[TAB]{   
[TAB][TAB]quoteAmount:标价资产  
[TAB][TAB]baseAmount:基准资产  
[TAB]}  
```  
...
参数：  
[TAB]assetId：需要注资的二级资产代币符号   
[TAB]amount：注资核心资产代币数量  
[TAB]onlyGetFee：设置只返回本次调用所需手续费  
[TAB]callback：回调函数  
...
功能：代币资产查询  
参数：  
[TAB]assetId：资产符号，此参数若为空，则查询链上发行的所有资产   
[TAB]callback：回调函数  
...
参数：  
[TAB]assetId：资产ID或代币符号，资产ID：数字代币的唯一代币标识ID（如："X.X.X"），代币符号（如：”BTC”）  
[TAB]account：用户名  
[TAB]callback：回调函数  
...
参数：  
[TAB]unit：记账单位，将会根据手续费汇率或交易市场价格换算等价的该资产，资产ID或代币符号，资产ID：数字代币的唯一代币标识ID（如："X.X.X"），代币符号（如：”BTC”）  
[TAB]account：用户名  
[TAB]callback：回调函数  
...
参数：  
[TAB]worldView：世界观名称，区分大小写;  
  
### 创造NH资产  
方法：creatNHAsset  
功能：创建一个唯一的NH资产，具有唯一性。本接口仅限NH资产制造商（铁匠铺）使用。  
参数：  
[TAB]assetId：当前NH资产交易时，适用的资产ID；  
[TAB]worldView：世界观；  
[TAB]baseDescribe：当前NH资产的具体内容描述，由制造者定义;  
[TAB]ownerAccount：指定NH资产拥有者(NH资产归属权账户，默认为NH资产创建者)  
[TAB]NHAssetsCount (Number)：创建NH资产的数量，默认值为1，只有type为0即创建同一种NH资产生效  
[TAB]type：创建NH资产方式的类型，默认值为0。值为0时默认创建同一种NH资产，1是创建不同NH资产。  
[TAB]NHAssets(Array)示例:   
```js  
[TAB][{  
[TAB] "assetId": "X.X.X",  
[TAB] "worldView": "TEST",  
[TAB] "baseDescribe": "{name:\"预言家\"}",  
[TAB] "ownerAccount": "test2"  
[TAB]}, {  
[TAB] "assetId": "X.X.X",  
[TAB] "worldView": "TEST",  
[TAB] "baseDescribe": "{name:\"女巫\"}",  
[TAB] "ownerAccount": "test2"  
[TAB]}, {  
[TAB] "assetId": "X.X.X",  
[TAB] "worldView": "TEST",  
[TAB] "baseDescribe": "{name:\"猎人\"}",  
[TAB] "ownerAccount": "test2"  
[TAB]}]  
```  
...
参数：  
[TAB]NHAssetIds (Array)：NH资产实例的唯一标识ID;示例：\[X.X.X, X.X.X\]  
  
### 转移NH资产  
方法：transferNHAsset  
功能：用户可以将自己的NH资产转移到另外一个用户  
参数：  
[TAB]toAccount：转移NH资产的目标用户名  
[TAB]NHAssetIds（Array）：多个NH资产id组成的数组，示例：\[X.X.X, X.X.X\]  
  
### 提议关联世界观  
方法：proposeRelateWorldView  
功能：提议关联到某一个世界观，需要该世界观的创建人审批  
参数：  
[TAB]worldView：需要关联的世界观名  
[TAB]viewOwner：需要关联的世界观创建者  
...
参数：  
[TAB]otcAccount：OTC交易平台账户，用于收取挂单费用；（OTC平台填写）  
[TAB]orderFee：挂单费用，用户向OTC平台账户支付的挂单费用；（OTC平台填写）  
[TAB]NHAssetId：NH资产实例的唯一标识ID; （用户填写）  
[TAB]price：商品挂单价格；（用户填写）  
[TAB]priceAssetId：商品挂单价格所使用的代币种类；（用户填写）  
[TAB]expiration：挂单时间，如3600(秒)，为1小时  
[TAB]memo：挂单备注信息；  
[TAB]callback：设置执行挂单卖出后的回调函数  
...
功能：查询全网用户NH资产的售卖订单   
参数：  
[TAB]assetSymbolsOrIds(array）：资产符号或id筛选条件  
[TAB]worldViews (array)：版本名称或版本id筛选条件  
[TAB]pageSize：页容量  
[TAB]page：页数  
...
参数：  
[TAB]account：账户名或账户id  
[TAB]worldViews (array)：世界观名集合  
[TAB]page：页数  
[TAB]pageSize：页容量，每页的数据条数  
[TAB]callback：返回值。示例：  
[TAB][TAB]{status:1,data:[],total:0}  
...
功能：查询NH资产详细信息   
参数：  
[TAB]NHAssetHashOrIds：NH资产id或hash  
[TAB]callback：回调函数  
...
功能：查询世界观详细信息   
参数：  
[TAB]worldViewNameOrIds (array)：世界观名称或id  
[TAB]callback：回调函数  
...
参数：  
[TAB]witnessesIds（array）：节点账户id集合，查询节点投票信息数据中会有每个节点的账户ID  
[TAB]proxyAccount：代理账户名  
[TAB]callback：回调函数  
...
功能：切换节点  
参数：  
[TAB]url：API服务器节点地址，此地址必须是API服务器节点列表中的websoket地址  
[TAB]callback：回调函数  
...
功能：监听rpc连接状态变化  
参数：  
[TAB]callback：回调会返回状态status，包括下述结果:   
[TAB]closed：rpc连接关闭  
[TAB]error：rpc连接错误  
[TAB]realopen：rpc连接成功  
...
参数：  
[TAB]authority：合约权限(一对公私钥中的公钥publicKey)，开发者在使用API初始化的时候，可以配置私钥，配置了该公钥对应的私钥才可以调用合约。  
[TAB]name：合约名称，规则/^\[a-z\]\[a-z0-9\\\.-\]\{4,\}/  
[TAB]data：合约lua代码  
[TAB]onlyGetFee：设置只返回本次操作所需手续费   
[TAB]callback：见统一API说明  
...
功能：更新合约代码  
参数：  
[TAB]nameOrId合约名称或Id，示例：contract.test02  
[TAB]data：合约lua代码  
[TAB]onlyGetFee：设置只返回本次操作所需手续费   
...
功能：调用合约函数接口   
参数：  
[TAB]nameOrId：合约名称或Id，示例：contract.test02  
[TAB]functionName：合约里的函数名称，my_nht_describe_change （修改道具属性）  
[TAB]valueList(array)： 调用合约函数的参数列表，示例：\['X.X.X','name','女巫'\]   
[TAB]runtime：运行合约函数的时间(单位毫秒)，默认为5  
[TAB]onlyGetFee：设置只返回本次操作所需手续费，默认为false  
[TAB]callback：回调函数  
...
功能：查询账户合约里产生数据  
参数：  
[TAB]userNameOrId：账户名或Id  
[TAB]contractNameOrId：合约名字或Id  
[TAB]callback：回调函数